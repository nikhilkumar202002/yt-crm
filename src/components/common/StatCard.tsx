...
export const StatCard = ({ label, value, icon, trend, colorClass = 'border-blue-500' }: StatCardProps) => (
  <div className={`bg-white p-4 rounded-xl shadow-sm border-l-2 ${colorClass} hover:shadow-md transition-shadow`}>
    <div className="flex justify-between items-center mb-3">
      <div className="p-1.5 bg-slate-50 rounded-md">{icon}</div>
      {trend && <span className="text-[9px] font-bold text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full">{trend}</span>}
    </div>
