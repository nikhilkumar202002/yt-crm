  ...
  return (
    <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-30 font-sans">
      ...
      <div className="flex items-center gap-4 flex-1">
        <button 
          onClick={onMenuClick}
          className="lg:hidden p-1.5 text-slate-500 hover:bg-slate-100 rounded-none transition-all"
        >
          <Menu size={18} />
        </button>
      ...
      <div className="flex items-center gap-3 lg:gap-5">
        ...
        <div className="flex items-center gap-1">
          ...
          <button title="Public Site" className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-none transition-all">
            <Globe size={15} />
          </button>

          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button className="relative p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-none transition-all cursor-pointer" title="Notifications">
                <Bell size={15} />
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-2 h-2 w-2 bg-red-500 border-2 border-white rounded-none" />
                )}
              </button>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content 
                className="min-w-[320px] max-w-[400px] bg-white rounded-none p-0 shadow-xl border border-slate-200 mt-2 animate-in fade-in zoom-in-95 duration-150 max-h-[400px] overflow-hidden"
                sideOffset={8}
                align="end"
              >
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <button className="flex items-center gap-2.5 group outline-none pl-2 border-l border-slate-100">
              ...
              <div className="h-8 w-8 rounded-none bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-600 font-bold text-xs shadow-sm">
                {user?.name?.charAt(0)}
              </div>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content 
              className="min-w-[180px] bg-white rounded-none p-1.5 shadow-xl border border-slate-200 mt-2 animate-in fade-in zoom-in-95 duration-150"
              sideOffset={8}
              align="end"
            >
