...
const Header = () => {
  ...
  return (
    <header className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-10 sticky top-0 z-30 font-sans">
      {/* Left: Breadcrumbs/Context */}
      <div className="flex items-center gap-4">
        <div className="h-8 w-[2px] bg-blue-600 rounded-full" />
        <div>
          <h2 className="text-lg font-bold text-gray-800 tracking-tight">Dashboard</h2>
          <p className="text-[10px] text-gray-400 uppercase font-bold tracking-[0.2em] leading-none mt-1">
            Performance & Intelligence {/* cite: 58 */}
          </p>
        </div>
      </div>
          ...
          <DropdownMenu.Trigger asChild>
            <button className="flex items-center gap-4 group outline-none">
              ...
              <div className="relative">
                <div className="h-11 w-11 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center text-white font-bold shadow-lg shadow-blue-200">
                  {user?.name?.charAt(0)}
                </div>
