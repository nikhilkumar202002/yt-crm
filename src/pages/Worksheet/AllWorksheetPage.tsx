...
import {
  Clipboard, Search, Upload, Edit, Trash2, Plus, LayoutGrid, List,
  Calendar as CalendarIcon, User as UserIcon, CheckCircle2, AlertCircle, FileText
} from 'lucide-react';
...
const AllWorksheetPage = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 font-sans">
      ...
      <div className={`relative ${viewMode === 'table' ? 'bg-white rounded-none shadow-sm border border-slate-200 overflow-hidden' : ''}`}>
        {loading ? (
                  ...
                  <div className="p-4 border-b border-slate-100 bg-slate-50 group-hover:bg-blue-50/30 transition-colors">
                    ...
                    <div className="flex items-center gap-2 text-slate-500">
                      ...
                      <span className="text-[11px] font-medium">
                        {work.date ? new Date(work.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : 'No Date'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 flex-1 space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          <button 
                            ...
                            onClick={() => { setUploadingWorkId(work.id); document.getElementById('admin-file-upload')?.click(); }}
                          >
                            <Upload size={12} />
                          </button>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                      <div className="space-y-1">
                        ...
                        <button 
                          onClick={() => setAssignmentModal({ isOpen: true, workId: work.id, initialIds: parseIds(work.assigned_to), type: 'designer' })}
                      ...
                      <div className="space-y-1">
                        ...
                        <button 
                          onClick={() => setAssignmentModal({ isOpen: true, workId: work.id, initialIds: parseIds(work.content_assigned_to), type: 'content' })}
                  ...
                  <div className="p-4 bg-slate-50 border-t border-slate-100 flex flex-col gap-2">
                    <div className="flex justify-between items-center gap-2">
                      <div className="flex-1">
                        <select 
                          value={contentStatuses[work.id] || work.status || 'pending'} 
                          onChange={(e) => handleContentStatusChange(work.id, e.target.value)}
                          className={`w-full text-[9px] font-black uppercase tracking-widest p-1.5 border-none rounded-none text-white ${(contentStatuses[work.id] || work.status) === 'completed' ? 'bg-green-600' : 'bg-slate-500'}`}
                        >
                          <option value="pending">Content: Pending</option>
                          <option value="working_progress">Content: In Progress</option>
                          <option value="completed">Content: Completed</option>
                      ...
                      <div className="flex-1">
                        <select 
                          value={designerStatuses[work.id] || work.designer_status || 'pending'} 
                          onChange={(e) => handleDesignerStatusChange(work.id, e.target.value)}
                          className={`w-full text-[9px] font-black uppercase tracking-widest p-1.5 border-none rounded-none text-white ${(designerStatuses[work.id] || work.designer_status) === 'completed' ? 'bg-green-600' : 'bg-slate-500'}`}
                        >
                          <option value="pending">Designer: Pending</option>
                          <option value="working_progress">Designer: In Progress</option>
                          <option value="completed">Designer: Completed</option>
                    ...
                    <div className="flex justify-between items-center bg-white p-2 border border-slate-200">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <select 
                          ...
                          className={`flex-1 text-[9px] font-bold outline-none cursor-pointer ${(clientApprovedStatuses[work.id] || work.client_approved_status) === 'approved' ? 'text-green-600' : 'text-slate-500'}`}
                        >
                          <option value="pending">Pending</option>
                          <option value="approved">Approved</option>
