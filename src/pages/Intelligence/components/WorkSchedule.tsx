        ...
        <table className="w-full text-left border-collapse border border-slate-200">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">
              <th className="px-6 py-3 border border-slate-200">Client</th>
              <th className="px-6 py-3 border border-slate-200">Task Description</th>
              <th className="px-6 py-3 border border-slate-200">Deadline</th>
              <th className="px-6 py-3 text-left border border-slate-200">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200">
            {loading ? (
              <tr><td colSpan={4} className="px-6 py-10 text-center text-[10px] text-slate-400 uppercase tracking-widest border border-slate-200">Loading schedule...</td></tr>
            ) : works.length > 0 ? (
              works.map((work) => (
                <tr key={work.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-[10px] text-slate-500 align-top border border-slate-200">
                    {work.date ? new Date(work.date).toLocaleDateString() : 'N/A'}
                  </td>
                  <td className="px-6 py-4 text-left align-top border border-slate-200">
                    <span className="px-2 py-0.5 bg-yellow-50 text-yellow-600 text-[9px] font-bold rounded-none border border-yellow-100">Pending</span>
                  </td>
                </tr>
              ))
            ) : (
              <tr><td colSpan={4} className="px-6 py-10 text-center text-[10px] text-slate-400 uppercase tracking-widest border border-slate-200">No tasks assigned for today</td></tr>
            )}
          </tbody>
        </table>
      </div>
