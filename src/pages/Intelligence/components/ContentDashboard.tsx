...
const ContentDashboard: React.FC<ContentDashboardProps> = ({ user, groupName }) => {
    ...
    const fetchCount = async () => {
      try {
        ...
        const filtered = data.filter((work: any) => {
          ...
          return parseIds(work.content_assigned_to).includes(userId);
        });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-1 bg-slate-900 p-6 rounded-none text-white flex flex-col justify-center relative overflow-hidden items-start">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Content Dept</p>
        ...
        <div className="md:col-span-1 bg-white p-5 rounded-none shadow-sm border border-slate-100 flex flex-col justify-center items-start">
          ...
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Assigned Content</p>
        </div>
      </div>
