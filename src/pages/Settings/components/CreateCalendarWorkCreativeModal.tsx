...
import { createCalendarWorkCreative } from '../../../api/services/microService';
import type { CalendarWorkCreativePayload } from '../../../api/services/microService';

interface CreateCalendarWorkCreativeModalProps {
  onSuccess: () => void;
}

export const CreateCalendarWorkCreativeModal = ({ onSuccess }: CreateCalendarWorkCreativeModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await createCalendarWorkCreative(formData);
      setIsOpen(false);
      onSuccess(); // Refresh the list
      setFormData({ name: '', description: '', status: true });
    } catch {
      alert('Failed to create calendar work creative');
    } finally {
      setLoading(false);
    }
  };
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl z-110 font-sans">
          <div className="flex justify-between items-center mb-6">
            <Dialog.Title className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
              <Palette size={16} className="text-blue-600" /> New Creative
            </Dialog.Title>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
              <input
                type="checkbox"
                id="create-creative-status"
                className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                checked={formData.status}
                onChange={(e) => setFormData({ ...formData, status: e.target.checked })}
              />
              <label htmlFor="create-creative-status" className="text-xs font-bold text-slate-700 cursor-pointer">Set as Active Creative</label>
            </div>
